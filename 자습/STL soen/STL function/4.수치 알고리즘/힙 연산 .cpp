...
단지, 첫 번째 요소가 제일 크기만을 요구하므로 나머지 뒷부분은 크기순으로 정렬되지 않아도 상관없다.
 요구 사항이 많지 않기 때문에 삽입, 삭제 연산도 훨씬 더 빠르다.
  힙 연산 함수들은 우선 순위 큐 구현에 흔히 사용되는데 
  사실 STL의 prority_queue 어댑터가 이런 자료 구조를 잘 제공하므로 힙 연산 함수들을 직접 사용할 일은 거의 없다. 


다음 4개의 함수가 힙 연산을 제공한다.
...
힙 연산은 우선 순위 큐같은 자료 구조를 구현하기 위한 연산이다
...
사실 힙 연산은 최종 사용자들이 쓸만한 연산은 아니며 실용성도 낮다.
