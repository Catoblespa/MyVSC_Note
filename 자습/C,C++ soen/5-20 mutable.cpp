...
public:
	...
	//그런데 때로는 객체의 멤버이면서도 객체의 상태에 포함되지 않는 멤버가 존재하기도 하는데 
	//예를 들어 값 교환을 위한 임시 변수가 이에 해당한다. 
	
	//또는 i, j같은 통상적인 루프 제어 변수도 객체의 상태라고 볼 수 없으며 
	//디버깅을 위해 임시적으로 추가된 멤버도 mutable이어야 한다. 
	//예를 들어 객체 상태를 출력해 보기 위한 문자열 버퍼를 멤버로 잠시 선언했다면 
	//이 버퍼는 객체의 주요 멤버 변수에 포함되지 않는다. 다음 예제를 보자.
